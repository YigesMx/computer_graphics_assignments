...
其实只需要在 Phong 光照模型的基础上，将法向量替换为从贴图中获取的法向量即可：
...
这是整个作业最麻烦的部分，因为它需要对整个渲染过程进行扩充，然而在实现上面的功能后代码已经非常冗长了，我也是在完成这个部分之前重构了代码，整合前面的 shader，并对代码进行重构封装。
...
这部分设计的代码众多，可以直接在代码中查看，这里只介绍一下主要的思路。